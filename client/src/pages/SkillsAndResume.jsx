...
export function SkillsAndResume() {
  ...
  function loadResumes() {
    listResumes()
      .then(setResumes)
      .catch((err) => notify.error('Could not load resumes', extractErrorMessage(err)));
  }
  ...
  return (
    <Layout title="Skills & resume">
      ...
      <div className="section-label" style={{ marginTop: 28 }}>Resumes</div>
      <p style={{ color: 'var(--text-dim)', fontSize: 13, marginTop: 0, marginBottom: 12 }}>
        Uploading a resume reads skills out of it automatically. Nothing is added to your
      ...
      {resumes !== null && resumes.length === 0 && (
        <p style={{ color: 'var(--text-dim)', fontSize: 13 }}>No resumes uploaded yet.</p>
      )}
      {resumes !== null && resumes.length > 0 && (
          ...
          {resumes.map((r) => (
            <div className="resume-row" key={r.id}>
              ...
              {r.proposedSkillCount > 0 && (
                <button className="pending-chip" onClick={() => setExtractingResume(r)}>
